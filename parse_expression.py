...
# 0.78 29/07/2019 : Support quoted string values
# 0.79 23/09/2019 : Include 'image' and 'image.alpha' function support, add mix(), cmix(), cadd(), csub(), cmult(), cdiv(), MixRGB() support
##################################################################################################################
...
#TODO: Come up with a syntax of using the other 'convert' modes (eg, HSV) (could just use [h], [s], [v], I guess) and introduce 'combine.hsv(...)'
...
class Expression():
    ...
    def valid_expression(expressionText):
        ...
        for v in values:
            if not Expression.is_number(v) and not (v.startswith('"') and v.endswith('"')):
    ...
    functionList = [('+',2,2),('-',2,2),('*',2,2),('/',2,2),('**',2,2),('sin',1,1),('cos',1,1),('tan',1,1),('asin',1,1),('acos',1,1),('atan',1,1),
        ('min',2,99),('max',2,99),('mod',2,2),('abs',1,1),('log',2,2),('round',1,1),('atan2',2,2),('not',1,1),('and',2,99),('or',2,99),('xor',2,99),('mmod',2,2),('combine',1,3),('clamp',1,1),('clip',1,1),('vadd',2,2),('vsub',2,2),('vmult',2,2),('vdiv',2,2),('vdot',2,2),('vcross',2,2),('vnorm',1,1),('noise',2,4),('floor',1,1),('fract',1,1),('ceil',1,1),('musgrave',2,7),('musgrave.fbm',2,7),('musgrave.mf',2,7),('musgrave.rmf',2,7),('musgrave.hmf',2,7),('musgrave.ht',2,7),('voronoi',2,2),('voronoi.cell',2,2), ('voronoi.crackle',2,2), ('voronoi.distance1',2,2), ('voronoi.distance2',2,2), ('voronoi.distance3',2,2), ('voronoi.distance4',2,2), ('colorramp',2,99),('group:start',1,99),('group:end',1,1),('group:add',2,99),('point', 2,9),('point.color', 2,9),('point.density', 2,9),('function:start',2,99),('function:end',1,1),('inputs',1,99),('outputs',1,99),('image',2,5),('image.alpha',2,5), ('mix', 3,3)]
        
    #TODO: Support 'MixRGB(<operation>, ....)' function
    #TODO: Add enhanced 'round' (allow step other than '1'), musgrave, etc.
    ...
    def is_function(function):
        if function.startswith("fn:") and "(" in function:     #TODO: Tighten up this check to only allow valid function names
            return True
    ...
    def balancedBrackets(str):
        ...
        if openCountRound != 0 or openCountSquare != 0 or openCountCurley != 0 or openQuote != 0:
            return False
        else:
            return True
        
    def stringRepresentation(_expr):
        """Convert a previously parsed expression into a string representation of that expression"""
        if len(_expr) <= 0:
            return ''
    ...
    def parse_expression(_expr):
                ...
                if Expression.balancedBrackets(_expr[openBracketPos+1:-1]):
                    if openBracketPos == 0:
                        ...
                        if len(expr)>0 and (expr[0] == ','):
                            if funcname == 'atan2':     #TODO: Change this to use 'native' ATAN2 if available. Also, look into implementing other new modes where available
                            ...
                            if funcname == 'not':
                                ...
                                return ('max',('min',expr, ('value','1.0')), ('value','0.0'))   # TODO: Implement as 'Clip' checkbox for these defaults
                                ...
                                return ('mod',('+',('mod',expr,('value','1.0')),('value','1.0')),('value','1.0'))  #TODO: Implement as new Maths node mode if available
                            elif funcname == 'floor':
                                #Return the next lower integer value
                                #Implemented as 'round(value - 0.5)'
                                return ('round',('-',expr,('value','0.5'))) #TODO: Implement as new Maths node mode if available
                            elif funcname == 'ceil':
        ...
        for i in reversed(range(0,len(_expr))):       # Next pass - addition
            ...
            if (i == 0) and (c == '+'):     # Ignore leading positive sign
                #_expr = _expr[1:]
                #i+=1
                #TODO: Can't assign values in strings like this - this causes an exception with an expression such as 'x + +y'
                _expr[0] = ' '
                continue
        ...
        if len(_expr) > 0:
            ...
            if _expr[0].isdigit() or (_expr[0].startswith('"') and _expr[0].endswith('"')):
                return ('value', _expr)
            else:
                return ('variable', _expr)
