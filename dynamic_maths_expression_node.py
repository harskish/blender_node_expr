...
# 1.40 29/10/2020 : Make ':condition: ....' prefix tolerant of ':' within the text
# 2.00 06/10/2021 : Attempt to extend to other node systems.
# 2.01 07/04/2022 : Compatibility with Blender 3.1, 3.2 with support for Compositor and Geometry nodes with differing presets
##################################################################################################################
...
#TODO: Get working for all relevant Compositor and Geometry nodes and change any Shader ones to use new node functionality (no need to support old styles any more (such as atan2))
#TODO: In compositor 'combine' should allow for 4th argument (can split out the 4th channel using c[a] or c[3]...although that should probably only be allowed in Compositor nodes)


#Added functionality for release 2.00 :
#   Enable in Compositor
#   Enable in Geometry Nodes
#   Allow colour[3]/colour[a] to reference the alpha channel when in Compositor
...
#2.00
#Function to get the string for the specified node without raising exception if not found
def checkNodeIdForCurrentTree(sourceNodeID):
    try:
        return getNodeIdForCurrentTree(sourceNodeID)
    except:
        return "Unknown"+sourceNodeID

def getNodeIdForCurrentTree(sourceNodeID):

    # Support Shader, Compositor, Geometry node trees
    if bpy.context.space_data.tree_type == "ShaderNodeTree":
        nodeEditorType = "Shader"
    elif bpy.context.space_data.tree_type == "CompositorNodeTree":
        nodeEditorType = "Compositor"
    ...
    # NodeGroup         -> <type>NodeGroup
    # NodeTree          -> <type>NodeTree
    if sourceNodeID == "NodeGroup" or sourceNodeID == "NodeTree" :
        if nodeEditorType == "Shader" or nodeEditorType == "Compositor" or nodeEditorType == "Geometry":
            return nodeEditorType + sourceNodeID
    
    # NodeMath -> ShaderNodeMath/CompositorNodeMath/ShaderNodeMath (for Shader/Compositor/Geometry)
    # NodeValToRGB (colorramp) -> ShaderNodeValToRGB/CompositorNodeValToRGB/ShaderNodeValToRGB (for Shader/Compositor/Geometry)
    # NodeMixRGB -> ShaderNodeMixRGB/CompositorNodeMixRGB/ShaderNodeMixRGB (for Shader/Compositor/Geometry)
    # NodeValue -> ShaderNodeValue/CompositorNodeValue/ShaderNodeValue (for Shader/Compositor/Geometry)
    if sourceNodeID == "NodeMath" or sourceNodeID == "NodeValToRGB" or sourceNodeID == "NodeMixRGB" or sourceNodeID == "NodeValue":
        if nodeEditorType == "Shader" or nodeEditorType == "Compositor":
            return nodeEditorType + sourceNodeID
        elif nodeEditorType == "Geometry":
            return "Shader" + sourceNodeID
    
    # NodeVectorMath -> *Shader*NodeVectorMath (for Shader/Geometry)
    if sourceNodeID == "NodeVectorMath":
        if nodeEditorType == "Shader" or nodeEditorType == "Geometry":
            return "Shader" + sourceNodeID

    # NodeValue         -> *Shader*NodeValue (for Shader/Geometry), CompositorNodeValue (for Compositor)
    if sourceNodeID == "NodeValue":
        if nodeEditorType == "Shader" or nodeEditorType == "Geometry":
            return "Shader" + sourceNodeID
        elif nodeEditorType == "Compositor":
            return "Compositor" + sourceNodeID
            
    # NodeSeparateXYZ   -> *Shader*NodeSeparateXYZ (for Shader/Geometry), CompositorNodeSepRGBA (for Compositor)
    if sourceNodeID == "NodeSeparateXYZ":
        if nodeEditorType == "Shader" or nodeEditorType == "Geometry":
            return "Shader" + sourceNodeID
        elif nodeEditorType == "Compositor":
            return "Compositor" + "NodeSepRGBA"
            
    # NodeCombineXYZ    -> *Shader*NodeCombineXYZ (for Shader/Geometry), CompositorNodeCombRGBA (for Compositor)
    if sourceNodeID == "NodeCombineXYZ":
        if nodeEditorType == "Shader" or nodeEditorType == "Geometry":
            return "Shader" + sourceNodeID
        elif nodeEditorType == "Compositor":
            return "Compositor" + "NodeCombRGBA"



    raise Exception("Unknown nodeID '"+sourceNodeID+"' for tree type '"+nodeEditorType+"'")
#2.00        
...
class _DynamicMathsExpression_Operator_common(bpy.types.Operator):
    ...
    def poll(cls, context):
        ...
        return (space.type == 'NODE_EDITOR') and ((space.tree_type == 'ShaderNodeTree') or (space.tree_type == 'CompositorNodeTree') or (space.tree_type == 'GeometryNodeTree')) and (hasattr(space,"node_tree")) and (hasattr(space.node_tree,"nodes"))

    def invoke(self, context, event):
        wm = context.window_manager
        
        viewWidth = int(bpy.context.area.width * 0.8)
        ...
        if self.mode == 'CREATE':
            if self.presetExpressionList != 'none':
                if self.presetAsTextBlock:
                    ...
                    print("Left expressionText un-decoded!")
                    self.expressionText = self.applyExpressionTextOptions(self.presetText, self.enabledOptions)
                    #RAS2022-05-19
    ...
    def __nodetree_setup__(self):
        ...
        for node in self.node_tree.nodes:
            if node.bl_idname == getNodeIdForCurrentTree("NodeGroup"):
            ...
            while True:
                ...
                if operations[0] == ',':
                    ...
                    if operations[1][0] == 'group:start' or operations[1][0] == 'function:start':
                        ...
                        self.node_tree = bpy.data.node_groups.new('group:start',getNodeIdForCurrentTree('NodeTree'))
                        ...
                        if operations[1][1][0] == 'variable':
                            ...
                            groupnode = self.node_tree.nodes.new(getNodeIdForCurrentTree('NodeGroup'))
    ...
    def build_nodes(self, nested_operations, to_output, output_location,depth,lastoperation=None, subExpressionNo=0, expressionName = ""):
        ...
        if operation == 'variable':
            ...
            if variableName.startswith('group:'):
                ...
                if variableSubscript != None:
                    ...
                    if separateNode == None:
                        separateNode = self.node_tree.nodes.new(getNodeIdForCurrentTree("NodeSeparateXYZ"))
                    ...
                    if variableSubscript in ('0', 'x', 'r'):
                        ...
                        subnum = 2
                    elif variableSubscript in ('3', 'a'):   # In compositor can have RGBA
                        subnum = 3
            ...
            newnode = self.node_tree.nodes.new(getNodeIdForCurrentTree("NodeValue"))
            ...
            newnode = self.node_tree.nodes.new(getNodeIdForCurrentTree("NodeCombineXYZ"))
            self.node_tree.links.new(newnode.outputs[0],to_output)
            self.storeNodeInCache(Expression.stringRepresentation(nested_operations), newnode)
            self.storeSubGroupNode(subExpressionNo, expressionName, newnode)
            ...
            # set the operation
            if operation == '+':
                nodetype = getNodeIdForCurrentTree("NodeMath")
                nodeop = "ADD"
            elif operation == '-':
                nodetype = getNodeIdForCurrentTree("NodeMath")
                nodeop = "SUBTRACT"
                ...
                nodetype = getNodeIdForCurrentTree("NodeVectorMath")
                nodeop = 'ADD'
            elif operation == 'vsub':
                nodetype = getNodeIdForCurrentTree("NodeVectorMath")
                nodeop = 'SUBTRACT'
                ...
                nodetype = getNodeIdForCurrentTree("NodeMixRGB")
                nodeop = "MIX"
                ...
                nodetype = getNodeIdForCurrentTree("NodeValToRGB")
            elif operation == ',':
                ...
                newnode = self.node_tree.nodes.new(getNodeIdForCurrentTree('NodeGroup'))
            ...
            if nodetype == checkNodeIdForCurrentTree("NodeMath") or nodetype == checkNodeIdForCurrentTree("NodeVectorMath"):
                newnode = self.node_tree.nodes.new(nodetype)
                newnode.operation = nodeop
            elif nodetype == checkNodeIdForCurrentTree("NodeMixRGB"):
                newnode = self.node_tree.nodes.new(nodetype)
            ...
            elif nodetype == getNodeIdForCurrentTree("NodeValToRGB"):      #ColorRamp
                newnode = self.node_tree.nodes.new(nodetype)
            ...
            if (nodetype.startswith('ShaderNodeTex') or nodetype == "(FunctionGroup)" or nodetype != checkNodeIdForCurrentTree("NodeMath"))and not skipSubExpressions:
            ####FIXED
            ...
            if len(output.name) > 0 and len(output.links) == 0:
                ...
                for input in self.node_tree.inputs:
                    try:
                        ...
                        #TODO : Handle this better - it seems output.name is sometimes not a valid UTF-8 string (in Blender 3.1.2). This logic just ignores them and carries on. Only seems to affect Geometry Nodes at Blender 3.1.2
                        pass
    ...
    def execute_main(self, context):
        ...
        if context.space_data.type == 'NODE_EDITOR' and context.space_data.node_tree is not None:
            ...
            if self.mode == 'CREATE':
                ...
                groupnode = get_active_node_tree(context).nodes.new(getNodeIdForCurrentTree('NodeGroup'))
                #RAS - It seems we have to do it this way for compatibility with Blender 3.1
                #groupnode.node_tree=bpy.data.node_groups.new("DynamicMathsExpressionGroup", getNodeIdForCurrentTree('NodeTree'))
                newtree = bpy.data.node_groups.new("DynamicMathsExpressionGroup", getNodeIdForCurrentTree('NodeTree'))
                newtree.interface_update(context)
                groupnode.node_tree = newtree
                #RAS
...
class DynamicMathsExpression_Operator(_DynamicMathsExpression_Operator_common):
    ...
    #Additional attributes used to store details during build
    groupnode = None
    node_tree = None
            ...
            for filename in os.listdir(presetsDir):
                ...
                if filename.endswith(".shader"):
                    if context.space_data.tree_type != "ShaderNodeTree":
                        continue
                if filename.endswith(".compositor"):
                    if context.space_data.tree_type != "CompositorNodeTree":
                        continue
                if filename.endswith(".geometry"):
...
class DynamicMathsExpressionToTextBlock_Operator(bpy.types.Operator):
    """Convert the expression to a text block"""
    
    bl_idname = "node.node_dynamic_maths_expression_to_text_block"
    bl_label = "Node Group - Dynamic Maths Expression(To Text Block)"
    bl_space_type = 'NODE_EDITOR'
    label = 'Dynamic Maths Expression(To Text Block)'
    bl_options = {'INTERNAL',}
    ...
    def execute(self, context):
        ...
        textblock = bpy.data.texts.new("ConvertedExpression")
        ...
        if 'Dynamic Maths Expression' in node_tree.nodes.keys():
            ...
            textblock.from_string(exprNode.expressionText)

            exprNode.storedValidExpression = exprNode.expressionText
            exprNode.expressionText = "TEXT:"+textblock.name

        else:
            textblock.from_string("(something went wrong - can't find expression node)")
        ...
        #....take the expression and put it in a text block, changing the expression to refer to it as 'TEXT:...'. Make the text block visible if there isn't one already present.
        #....need to write this bit. May also need to make it not based on ...._common... but need to have the expression node available to manipulate it.
...
class DynamicMathsExpressionNode(bpy.types.NodeCustomGroup):
    ...
    def init(self, context):
        #print("Node Init")
        self.node_tree=bpy.data.node_groups.new(self.bl_name, getNodeIdForCurrentTree('NodeTree'))
    ...
    def draw_buttons(self, context, layout):
        ...
        if self.expressionText.startswith("TEXT:"):
            ...
            row=layout.row()
            row.operator('node.node_dynamic_maths_expression_to_text_block', text='Convert expression to Text block').node_tree_name = self.id_data.name; #Get group this node resides in
